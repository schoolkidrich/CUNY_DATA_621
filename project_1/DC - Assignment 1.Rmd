...
```{r}
url2 <- 'https://raw.githubusercontent.com/Jagdish16/CUNY_DATA_621/main/project_1/moneyball-training-data.csv'

moneyball_test <- read.csv(url2)

moneyball_test <- moneyball_test[-c(1, 10, 11)]
...
moneyball_test <- cbind(moneyball_test[1:2], moneyball_test[15], moneyball_test[3:14])
...
summary(moneyball_test$prediction)


moneyball_test <- moneyball_test %>%
  mutate(pred_diff = WINS - prediction)


ggplot(moneyball_test, aes(x = pred_diff)) + 
  geom_histogram(color = 'black', fill =  'gray') +
  geom_vline(aes(xintercept = mean(pred_diff)),
             linetype = 'dashed', size = 2, color = 'blue') +
  geom_label(aes(x = -25, y = 125,
                 label = str_replace_all(toString(summary(moneyball_test['pred_diff'])), ',', '\n')
                 )) +
  labs(title = 'Double Plays Histogram Plot', y = 'Count', x = 'Double Plays')
```
